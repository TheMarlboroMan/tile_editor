...
std::vector<std::string> Herramientas::explotar(const std::string & p_cadena, const char p_delimitador, size_t max)
{
	size_t cuenta=0;
...
std::vector<unsigned int> Herramientas::explotar_linea_a_enteros(const std::string& linea, const char separador)
{
	std::vector<unsigned int> resultado;

	std::vector<std::string> cadenas=DLibH::Herramientas::explotar(linea, separador);
	for(const std::string& c : cadenas) resultado.push_back(std::atoi(c.c_str()));
	return resultado;
}

std::string Herramientas::obtener_siguiente_linea_archivo(std::ifstream& archivo, char comentario)
{
	if(archivo.eof())
	{
		return "";
	...
	else
	{
		...
		if(!cadena.size() || cadena[0]==comentario) return obtener_siguiente_linea_archivo(archivo, comentario);
		else return cadena;
	}
}
