...
void Representacion_texto::establecer_dimensiones_area(unsigned int p_ancho, unsigned int p_alto)
{
	...
	establecer_posicion(0, 0, ancho_area, alto_area, FRECT_W|FRECT_H);	//Esto debemos llamarlo aquí, de lo contrario se queda con ancho y alto 0, dando problemas con las cámaras.
...
{
	interno_asignar(p_param);	
}

void Representacion_texto::interno_asignar(const std::string& temp)
{
	if(temp!=cadena)
	{
		cadena=temp;
